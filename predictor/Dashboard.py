...
def fetch_stock_data(ticker_symbol, start_date, end_date, interval_time):
    data = yf.download(ticker_symbol, start=start_date, end=end_date, interval=interval_time)
    return data

def fetch_analysis(stock_symbol, start_date_str, end_date_str, interval='1mo'):
    end_date = datetime.now()

    # Calculate the start date for the previous month's first date
    if end_date.month == 1:
        start_date = datetime(end_date.year - 1, 12, 1)
    else:
        start_date = datetime(end_date.year, end_date.month - 1, 1)

    # Format start_date and end_date as strings in the format YYYY-MM-DD
    start_date_str = start_date.strftime("%Y-%m-%d")
    end_date_str = end_date.strftime("%Y-%m-%d")
    analysis = yf.download(stock_symbol, start=start_date_str, end=end_date_str, interval=interval)
    
    fig2 = go.Figure()
    fig2.add_trace(go.Candlestick(x=analysis.index,
                        open=analysis['Open'],
                        high=analysis['High'],
                        low=analysis['Low'],
...
st.title("Trading Monk")
st.write("Welcome to the PlayGround!")
st.write("Trading Monk is a stock market prediction bot from India, designed to assist beginners in the market with stock picking and investment. It utilizes data from Yahoo Finance, which you can access at <https://finance.yahoo.com/>.")
...
start_date = st.date_input("Select Start Date", datetime(2024, 1, 1))
interval_time = st.selectbox(
    'Select an option',
    ('1m', '2m', '5m', '15m', '30m', '60m', '90m', '1h', '1d', '5d', '1wk', '1mo', '3mo')
    )

session_data = yf.download(user_input, start=start_date, end= datetime.now().date())
funda_data = yf.Ticker(user_input)

st.sidebar.title("Tools")
if st.button("Retrieve Data"):
    end_date = datetime.now().date()  # Accessing today's date properly
    data = fetch_stock_data(user_input, start_date, end_date, interval_time)
    ...
    fetch_analysis(user_input, 0, 0, interval='1mo')
            
            
            
    # Add some options to the sidebar as buttons
price_data = st.sidebar.button("Pricing Data")
fundamental_data = st.sidebar.button("Fundamental Data")

    # Main content based on button clicks
if price_data:
    st.title("Pricing Data")
    data2 = session_data
    data2['% Change'] = session_data['Adj Close'] / session_data['Adj Close'].shift(1) - 1
    ...
    st.write('Risk Adj. Return is ', annual_return/(stdev*100))
        ...
        # Add your Python code here to execute when the Home button is clicked
        # For example:
    ...
    fundamental_data = funda_data.info
    keys_of_interest = [
        'previousClose', 
        'marketCap', 
        'forwardPE', 
        ...
        'totalShareholderEquity',
            ...
            # Filter the fundamental data dictionary
    filtered_fundamental_data = {key: fundamental_data[key] for key in keys_of_interest if key in fundamental_data}

        # Display the filtered fundamental data in a tabular format

    for key, value in filtered_fundamental_data.items():
        st.write(key, value)
